...
func (api *BlockChainAPI) rpcMarshalHeader(ctx context.Context, header *types.Header) map[string]interface{} {
	...
	fields["totalDifficulty"] = (*hexutil.Big)(api.b.GetTd(ctx, header.Hash())) // ##CROSS: legacy sync
	return fields
}
...
func (api *BlockChainAPI) rpcMarshalBlock(ctx context.Context, b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	if inclTx {
		fields["totalDifficulty"] = (*hexutil.Big)(api.b.GetTd(ctx, b.Hash())) // ##CROSS: legacy sync
	}
	return fields, nil
}
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, blockTime uint64, index uint64, baseFee *big.Int, config *params.ChainConfig) *RPCTransaction {
	...
	case types.FeeDelegatedDynamicFeeTxType: // ##CROSS: fee delegation
		al := tx.AccessList()
		result.Accesses = &al
		result.ChainID = (*hexutil.Big)(tx.ChainId())
		result.GasFeeCap = (*hexutil.Big)(tx.GasFeeCap())
...
func (api *TransactionAPI) currentBlobSidecarVersion() byte {
	...
	// ##CROSS: istanbul
	// Istanbul consensus does not support PeerDAS yet, so version 0 is used.
	if !api.b.ChainConfig().IsIstanbulConsensus() && api.b.ChainConfig().IsOsaka(h.Number, h.Time) {
...
// ##CROSS: gas abstraction
func (api *TransactionAPI) applyGasAbs(ctx context.Context, tx *types.Transaction) (*types.Transaction, error) {
	if api.gasAbs == nil {
		return tx, nil
	}
	...
	// If it is approved, try to sign with the fee payer.
	if approved {
