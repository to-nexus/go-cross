...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --zonezero, --sepolia, --holesky, --hoodi instead)",
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	urls := params.CrossBootnodes // ##CROSS: config
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	flags.CheckExclusive(ctx, CrossFlag, ZoneZeroFlag, CrossDev3Flag, CrossDevFlag, MainnetFlag, DeveloperFlag, SepoliaFlag, HoleskyFlag, HoodiFlag, NetworkIdFlag, OverrideGenesisFlag) // ##CROSS: config
	...
	case ctx.Bool(DeveloperFlag.Name):
		...
		} else if cfg.Miner.Etherbase != (common.Address{}) { // ##CROSS: legacy sync
