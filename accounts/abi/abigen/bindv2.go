...
func BindV2(types []string, abis []string, bytecodes, runtimebytecodes []string, pkg string, libs map[string]string, aliases map[string]string) (string, error) {
	// ##CROSS: abigen
	// runtimebytecodes is added
	...
	for i := 0; i < len(types); i++ {
		...
		// ##CROSS: abigen
		var binRuntime string // runtimebytecodes can be nil
		if i < len(runtimebytecodes) {
			binRuntime = runtimebytecodes[i]
		}
		b.contracts[types[i]] = newTmplContractV2(types[i], abis[i], bytecodes[i], binRuntime, evmABI.Constructor, cb)
	}
