...
func (p *SignedRoundChangePayload) DecodeRLP(stream *rlp.Stream) error {
	...
	if err != nil {
		log.Error("Istanbul: Error Raw()", "err", err)
		return err
	}
	...
	if _, err = payloadStream.List(); err != nil {
		log.Error("Istanbul: Error List() Payload", "err", err)
		return err
	}

	if err = payloadStream.Decode(&p.Sequence); err != nil {
		log.Error("Istanbul: Error Decode(&m.Sequence)", "err", err)
		return err
	}
	if err = payloadStream.Decode(&p.Round); err != nil {
		log.Error("Istanbul: Error Decode(&m.Round)", "err", err)
		return err
	}
	...
	if size, err = payloadStream.List(); err != nil {
		log.Error("Istanbul: Error List() Prepared", "err", err)
		return err
	}
	if size > 0 {
		if err = payloadStream.Decode(&p.PreparedRound); err != nil {
			log.Error("Istanbul: Error Decode(&m.PreparedRound)", "err", err)
			return err
		}
		if err = payloadStream.Decode(&p.PreparedDigest); err != nil {
			log.Error("Istanbul: Error Decode(&p.PreparedDigest)", "err", err)
			return err
		}
	}
	// End Prepared
	...
	log.Info("Istanbul: Correctly decoded SignedRoundChangePayload", "p", p)
...
func (m *RoundChange) DecodeRLP(stream *rlp.Stream) error {
	...
	// Signed Payload
	if _, err = stream.List(); err != nil {
		log.Error("Istanbul: Error List() Signed Payload", "struct", "RoundChange", "err", err)
		return err
	}
	...
	if size > 0 {
		...
		if err = payloadStream.Decode(&m.PreparedDigest); err != nil {
			log.Error("Istanbul: Error Decode(&m.PreparedDigest)", "err", err)
			return err
		}
	}
	// End Prepared
	...
	if _, size, err = stream.Kind(); err != nil {
		log.Error("Istanbul: Error Kind()", "err", err)
		return err
	}
	if size == 0 {
		...
		if m.PreparedBlock.Hash() != m.PreparedDigest {
			log.Error("Istanbul: Error m.PreparedDigest.Hash() != digest")
			return istanbul.ErrFailedDecodePreprepare
		}
	}
	...
	if size == 0 {
		...
		if err = stream.Decode(&m.Justification); err != nil {
			log.Error("Istanbul: Error Decode(&m.Justification)", "err", err)
			return err
		}
	}
