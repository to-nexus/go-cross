...
func newRoundState(view *istanbul.View, validatorSet istanbul.ValidatorSet, preprepare *protocols.Preprepare, preparedRound *big.Int, preparedBlock istanbul.Proposal, pendingRequest *Request, hasBadProposal func(hash common.Hash) bool) *roundState {
	return &roundState{
		round:          view.Round,
		sequence:       view.Sequence,
		Preprepare:     preprepare,
		Prepares:       newMsgSet(validatorSet),
		Commits:        newMsgSet(validatorSet),
...
type roundState struct {
	round      *big.Int
	sequence   *big.Int
	Preprepare *protocols.Preprepare
...
func (s *roundState) SetPreprepare(preprepare *protocols.Preprepare) {
	s.mu.Lock()
	defer s.mu.Unlock()
