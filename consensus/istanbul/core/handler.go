...
func (c *Core) Stop() error {
	c.logger.Info("Istanbul: stopping...")
	c.stopTimer()
	c.unsubscribeEvents()

	// Make sure the handler goroutine exits
	c.handlerWg.Wait()
	c.logger.Info("Istanbul: stopped")
	return nil
}
	...
	for {
		...
		case event, ok := <-c.events.Chan():
			...
			case backlogEvent:
				...
				if err != nil {
					c.logger.Error("Istanbul: can not encode backlog message", "err", err)
					continue
				}
...
func (c *Core) deliverMessage(m protocols.Message) error {
	...
	switch m.Code() {
	case protocols.PreprepareCode:
		err = c.handlePreprepareMsg(m.(*protocols.Preprepare))
	case protocols.PrepareCode:
		err = c.handlePrepare(m.(*protocols.Prepare))
	case protocols.CommitCode:
		err = c.handleCommitMsg(m.(*protocols.Commit))
	case protocols.RoundChangeCode:
		err = c.handleRoundChange(m.(*protocols.RoundChange))
	default:
		c.logger.Error("Istanbul: invalid message code", "code", m.Code())
...
func (c *Core) handleTimeoutMsg() {
	...
	//logger.Warn("Istanbul: TIMER CHANGING ROUND", "pr", c.current.preparedRound)
	c.startNewRound(nextRound)
	logger.Warn("Istanbul: TIMER CHANGED ROUND", "pr", c.current.preparedRound)
...
func (c *Core) currentLogger(state bool, msg protocols.Message) log.Logger {
	logCtx := []interface{}{}
	if c.current != nil {
		...
		if proposer := c.valSet.GetProposer(); proposer != nil {
			logCtx = append(logCtx, "current.proposer", c.valSet.GetProposer().Address())
		}
	}
...
func withMsg(logger log.Logger, msg protocols.Message) log.Logger {
	return logger.New(
		"msg.code", msg.Code(),
		"msg.source", msg.Source().String(),
		"msg.round", msg.View().Round.Uint64(),
