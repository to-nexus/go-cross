...
func testParameterizedCase(
	...
	validatorSet := validator.NewSet(generateValidators(quorumSize), nil, pp)
	block := makeBlock(1)
	var round int64 = 10
	...
	roundChangeMessages := make([]*protocols.SignedRoundChangePayload, 0)
	for index, validator := range validatorSet.List() {
		var m *protocols.SignedRoundChangePayload
		if index < rcForNil {
	...
	// PREPARE messages
	prepareMessages := make([]*protocols.Prepare, 0)
	for index, validator := range validatorSet.List() {
		var m *protocols.Prepare
...
func createPrepareMessage(from common.Address, round int64, preparedBlock istanbul.Proposal) *protocols.Prepare {
	return protocols.NewPrepareWithSigAndSource(big.NewInt(1), big.NewInt(round), preparedBlock.Hash(), nil, from)
}
