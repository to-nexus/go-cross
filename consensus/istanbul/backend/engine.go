...
func (sb *Backend) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if len(addresses) > 0 {
		...
		if err != nil {
			log.Error("Istanbul: error writing validator vote", "err", err)
			return err
		}
	}
...
func (sb *Backend) Start(chain consensus.ChainHeaderReader, currentBlock func() *types.Block, hasBadBlock func(db ethdb.Reader, hash common.Hash) bool) error {
	...
	log.Info("start Istanbul")
	err := sb.startIstanbul()
...
func (sb *Backend) storeSnap(snap *Snapshot) error {
	...
	logger.Debug("Istanbul: store snapshot to database")
	if err := snap.store(sb.db); err != nil {
		logger.Error("Istanbul: failed to store snapshot to database", "err", err)
		return err
	}
...
func (sb *Backend) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if s, ok := sb.recents.Get(hash); ok {
			snap = s
			sb.snapLogger(snap).Trace("Istanbul: loaded voting snapshot from cache")
			break
		}
		...
		if number%checkpointInterval == 0 {
			if s, err := loadSnapshot(sb.config.GetConfig(new(big.Int).SetUint64(number)).Epoch, sb.db, hash); err == nil {
				snap = s
				sb.snapLogger(snap).Trace("Istanbul: loaded voting snapshot from database")
				break
			}
		}
		...
		if number == 0 {
			...
			if err := sb.Engine().VerifyHeader(chain, genesis, nil, nil); err != nil {
				sb.logger.Error("Istanbul: invalid genesis block", "err", err)
				return nil, err
			}
	...
	// When snapshot is called by ImportChain, parent headers might not be stored in DB yet.
	// In this case, snapApply will fail without passing the base parent explicitly.
	// So, we find current snap's header as base parent and pass it to snapApply.
