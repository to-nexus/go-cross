...
// bls12Signature used in the BLS signature scheme.
...
func signatureFromBytesNoValidation(sig []byte) (*blstSignature, error) {
	if len(sig) != common.SignatureLength {
		return nil, fmt.Errorf("signature must be %d bytes", common.SignatureLength)
	}
...
func SignatureFromBytesNoValidation(sig []byte) (common.Signature, error) {
	...
	return &bls12Signature{s: signature}, nil
}
...
func AggregateCompressedSignatures(multiSigs [][]byte) (common.Signature, error) {
	...
	return &bls12Signature{s: signature.ToAffine()}, nil
}
...
func MultipleSignaturesFromBytes(multiSigs [][]byte) ([]common.Signature, error) {
	...
	for _, s := range multiSigs {
		if len(s) != common.SignatureLength {
			return nil, fmt.Errorf("signature must be %d bytes", common.SignatureLength)
		}
	}
	...
	for i, signature := range multiSignatures {
		...
		copiedSig := signature
		wrappedSigs[i] = &bls12Signature{s: copiedSig}
	}
...
func (s *bls12Signature) Verify(pubKey common.PublicKey, msg []byte) bool {
	// Signature and PKs are assumed to have been validated upon decompression!
	return s.s.Verify(false, pubKey.(*bls12PublicKey).p, false, msg, dst)
}
...
func (s *bls12Signature) FastAggregateVerify(pubKeys []common.PublicKey, msg [32]byte) bool {
	if len(pubKeys) == 0 {
		return false
	}
	rawKeys := make([]*blstPublicKey, len(pubKeys))
	for i := 0; i < len(pubKeys); i++ {
		rawKeys[i] = pubKeys[i].(*bls12PublicKey).p
	}
...
func (s *bls12Signature) Eth2FastAggregateVerify(pubKeys []common.PublicKey, msg [32]byte) bool {
...
func AggregateSignatures(sigs []common.Signature) common.Signature {
	...
	for i := 0; i < len(sigs); i++ {
		rawSigs[i] = sigs[i].(*bls12Signature).s
	}

	// Signature and PKs are assumed to have been validated upon decompression!
	signature := new(blstAggregateSignature)
	signature.Aggregate(rawSigs, false)
	return &bls12Signature{s: signature.ToAffine()}
}
...
func VerifyMultipleSignatures(sigs [][]byte, msgs [][32]byte, pubKeys []common.PublicKey) (bool, error) {
	...
	for i := 0; i < length; i++ {
		mulP1Aff[i] = pubKeys[i].(*bls12PublicKey).p
		rawMsgs[i] = msgs[i][:]
	}
...
func (s *bls12Signature) Marshal() []byte {
	return s.s.Compress()
}

// Copy returns a full deep copy of a signature.
func (s *bls12Signature) Copy() common.Signature {
	sign := *s.s
	return &bls12Signature{s: &sign}
}
