...
func TestFastAggregateVerify(t *testing.T) {
	...
	assert.True(t, aggSig.FastAggregateVerify(pubkeys, msg), "Signature did not verify")
...
func TestVerifyCompressed(t *testing.T) {
	...
	msg := []byte("hello")
	sig := priv.Sign(msg)
	assert.True(t, sig.Verify(pub, msg), "Non compressed signature did not verify")
	assert.True(t, VerifyCompressed(sig.Marshal(), pub.Marshal(), msg), "Compressed signatures and pubkeys did not verify")
}
...
func TestVerifySingleSignature_InvalidSignature(t *testing.T) {
	...
	assert.False(t, valid, "Signature did verify")
}
...
func TestVerifySingleSignature_ValidSignature(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.True(t, valid, "Signature did not verify")
}
...
func TestMultipleSignatureVerification(t *testing.T) {
	...
	assert.True(t, verify, "Signature did not verify")
}
...
func TestFastAggregateVerify_ReturnsFalseOnEmptyPubKeyList(t *testing.T) {
	...
	assert.False(t, aggSig.FastAggregateVerify(pubkeys, msg), "Expected FastAggregateVerify to return false with empty input ")
}
...
func TestEth2FastAggregateVerify(t *testing.T) {
	...
	aggSig := AggregateSignatures(sigs)
	assert.True(t, aggSig.Eth2FastAggregateVerify(pubkeys, msg), "Signature did not verify")
...
func TestEth2FastAggregateVerify_ReturnsFalseOnEmptyPubKeyList(t *testing.T) {
	...
	aggSig := NewAggregateSignature()
	assert.False(t, aggSig.Eth2FastAggregateVerify(pubkeys, msg), "Expected Eth2FastAggregateVerify to return false with empty input")
}
...
func TestEth2FastAggregateVerify_ReturnsTrueOnG2PointAtInfinity(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, aggSig.Eth2FastAggregateVerify(pubkeys, msg))
}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.err != nil {
				...
				assert.NoError(t, err)
				for i, s := range res {
					assert.Equal(t, s.Marshal(), test.input[i])
				}
			}
		})
	}
...
func TestCopy(t *testing.T) {
	...
	signatureA := &bls12Signature{s: new(blstSignature).Sign(key.p, []byte("foo"), dst)}
	signatureB, ok := signatureA.Copy().(*bls12Signature)
	require.Equal(t, true, ok)

	assert.NotSame(t, signatureA, signatureB)
	assert.NotSame(t, signatureA.s, signatureB.s)
	assert.Equal(t, signatureA.Marshal(), signatureB.Marshal())

	signatureA.s.Sign(key.p, []byte("bar"), dst)
	assert.NotEqual(t, signatureA.Marshal(), signatureB.Marshal())
}
