...
var pubkeyCache = lru.NewCache[[common.PublicKeyLength]byte, common.PublicKey](2_000_000)

// bls12PublicKey used in the BLS signature scheme.
...
func publicKeyFromBytes(pubKey []byte, cacheCopy bool) (common.PublicKey, error) {
	if len(pubKey) != common.PublicKeyLength {
		return nil, fmt.Errorf("public key must be %d bytes", common.PublicKeyLength)
	}
	newKey := (*[common.PublicKeyLength]byte)(pubKey)
...
func AggregatePublicKeys(pubs [][]byte) (common.PublicKey, error) {
	...
	for _, pubkey := range pubs {
		...
		mulP1 = append(mulP1, pubKeyObj.(*bls12PublicKey).p)
	}
	...
	return &bls12PublicKey{p: agg.ToAffine()}, nil
}

// Marshal a public key into a LittleEndian byte slice.
func (p *bls12PublicKey) Marshal() []byte {
	return p.p.Compress()
}

// Copy the public key to a new pointer reference.
func (p *bls12PublicKey) Copy() common.PublicKey {
	np := *p.p
	return &bls12PublicKey{p: &np}
}

// IsInfinite checks if the public key is infinite.
func (p *bls12PublicKey) IsInfinite() bool {
...
// the current one.
func (p *bls12PublicKey) Equals(p2 common.PublicKey) bool {
	return p.p.Equals(p2.(*bls12PublicKey).p)
}

// Aggregate two public keys.
func (p *bls12PublicKey) Aggregate(p2 common.PublicKey) common.PublicKey {
	agg := new(blstAggregatePublicKey)
	// No group check here since it is checked at decompression time
	agg.Add(p.p, false)
	agg.Add(p2.(*bls12PublicKey).p, false)
	p.p = agg.ToAffine()
...
func AggregateMultiplePubkeys(pubkeys []common.PublicKey) common.PublicKey {
	...
	for _, pubkey := range pubkeys {
		mulP1 = append(mulP1, pubkey.(*bls12PublicKey).p)
	}
	agg := new(blstAggregatePublicKey)
	// No group check needed here since it is done in PublicKeyFromBytes
	// Note the checks could be moved from PublicKeyFromBytes into Aggregate
	// and take advantage of multi-threading.
	agg.Aggregate(mulP1, false)
	return &bls12PublicKey{p: agg.ToAffine()}
}
