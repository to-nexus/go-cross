...
const (
	...
	protocolMaxMsgSize = 10 * 1024 * 1024 // Maximum cap on the size of a protocol message // ##CROSS: istanbul
)

var errMsgTooLarge = errors.New("message too long") // ##CROSS: istanbul
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, h.chain, eth.BlockRangeUpdatePacket{}, h.permission); err != nil { // ##CROSS: disable new sync
		...
		select {
		case peer.EthPeerDisconnected <- struct{}{}: // ##CROSS: istanbul
		default:
		}
		return err
	}
	...
	p.EthPeerRegistered <- struct{}{} // ##CROSS: istanbul
...
func (h *handler) Stop() {
	...
	// h.blockRange.stop() // ##CROSS: disable new sync
	// h.txFetcher.Stop() // ##CROSS: legacy sync
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) { // ##CROSS: legacy sync
...
func (h *handler) minedBroadcastLoop() { // ##CROSS: legacy sync
	defer h.wg.Done()
