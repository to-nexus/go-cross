...
type NewBlockPacket struct {
	Block    *types.Block
	TD       *big.Int
	Sidecars types.BlobSidecars `rlp:"optional"` // ##CROSS: blob sidecars
}
...
type BlockBody struct {
	...
	Sidecars     types.BlobSidecars   `rlp:"optional"` // Sidecars contained within a block // ##CROSS: blob sidecars
}

// Unpack retrieves the transactions and uncles from the range packet and returns
// them in a split flat format that's more consistent with the internal data structures.
func (p *BlockBodiesResponse) Unpack() ([][]*types.Transaction, [][]*types.Header, [][]*types.Withdrawal, []types.BlobSidecars) { // ##CROSS: blob sidecars
	var (
		txset         = make([][]*types.Transaction, len(*p))
		uncleset      = make([][]*types.Header, len(*p))
		withdrawalset = make([][]*types.Withdrawal, len(*p))
		sidecarset    = make([]types.BlobSidecars, len(*p)) // ##CROSS: blob sidecars
	)
	for i, body := range *p {
		// ##CROSS: blob sidecars
