...
const (
	...
	sidecarsCacheLimit = 256 // ##CROSS: blob sidecars
...
type BlockChain struct {
	...
	// ##CROSS: istanbul
	currentBlockWithBody atomic.Pointer[types.Block] // Current block of the chain
	...
	sidecarsCache *lru.Cache[common.Hash, types.BlobSidecars] // ##CROSS: blob sidecars

	// future blocks are blocks added for later processing
	futureBlocks *lru.Cache[common.Hash, *types.Block] // ##CROSS: legacy sync

	wg            sync.WaitGroup // ##CROSS: legacy sync
...
func NewBlockChain(db ethdb.Database, genesis *Genesis, engine consensus.Engine, cfg *BlockChainConfig) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		sidecarsCache: lru.NewCache[common.Hash, types.BlobSidecars](sidecarsCacheLimit), // ##CROSS: blob sidecars
		blockCache:    lru.NewCache[common.Hash, *types.Block](blockCacheLimit),
		txLookupCache: lru.NewCache[common.Hash, txLookup](txLookupCacheLimit),
		futureBlocks:  lru.NewCache[common.Hash, *types.Block](maxFutureBlocks), // ##CROSS: legacy sync
		engine:        engine,
...
func (bc *BlockChain) loadLastState() error {
	...
	if headBlock.Hash() != currentSnapBlock.Hash() {
		snapTd := bc.GetTd(currentSnapBlock.Hash(), currentSnapBlock.Number.Uint64()) // ##CROSS: legacy sync
	...
	if currentFinalBlock != nil {
		finalTd := bc.GetTd(currentFinalBlock.Hash(), currentFinalBlock.Number.Uint64()) // ##CROSS: legacy sync
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	delFn := func(db ethdb.KeyValueWriter, hash common.Hash, num uint64) {
		...
		if num+1 <= frozen {
			...
			rawdb.DeleteBlobSidecars(db, hash, num) // ##CROSS: blob sidecars
			rawdb.DeleteReceipts(db, hash, num)
			rawdb.DeleteTd(db, hash, num) // ##CROSS: legacy sync
		}
	...
	bc.sidecarsCache.Purge() // ##CROSS: blob sidecars
	bc.blockCache.Purge()
	bc.txLookupCache.Purge()
	bc.futureBlocks.Purge() // ##CROSS: legacy sync
...
func (bc *BlockChain) ResetWithGenesisBlock(genesis *types.Block) error {
	...
	rawdb.WriteTd(batch, genesis.Hash(), genesis.NumberU64(), genesis.Difficulty()) // ##CROSS: legacy sync
...
func (bc *BlockChain) procFutureBlocks() { // ##CROSS: legacy sync
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []rlp.RawValue, ancientLimit uint64) (int, error) {
	...
	if bc.chainConfig.IsIstanbulConsensus() && bc.chainConfig.IsCancun(lastBlk.Number(), lastBlk.Time()) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []rlp.RawValue) (int, error) {
		...
		if first.NumberU64() == 1 {
			if frozen, _ := bc.db.Ancients(); frozen == 0 {
				td := bc.genesisBlock.Difficulty() // ##CROSS: legacy sync
		...
		// ##
		writeSize, err := rawdb.WriteAncientBlocksWithBlobs(bc.db, blockChain, receiptChain, td) // ##CROSS: blob sidecars
		if err != nil {
	...
	writeLive := func(blockChain types.Blocks, receiptChain []rlp.RawValue) (int, error) {
		...
		for i, block := range blockChain {
			tdSum.Add(tdSum, block.Difficulty()) // ##CROSS: legacy sync
			...
			rawdb.WriteTd(batch, block.Hash(), block.NumberU64(), tdSum) // ##CROSS: legacy sync
			rawdb.WriteCanonicalHash(batch, block.Hash(), block.NumberU64())
...
func (bc *BlockChain) writeBlockWithoutState(block *types.Block, td *big.Int) (err error) {
	...
	batch := bc.db.NewBatch()
	rawdb.WriteTd(batch, block.Hash(), block.NumberU64(), td) // ##CROSS: legacy sync
	rawdb.WriteBlock(batch, block)
	// ##CROSS: blob sidecars
...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent, precheckDuplicate bool) (status WriteStatus, err error) {
	...
	bc.futureBlocks.Remove(block.Hash()) // ##CROSS: legacy sync
	...
	// ##CROSS: istanbul
	if bc.chainConfig.IsIstanbulConsensus() {
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator, makeWitness bool) (*stateless.Witness, int, error) { // ###CROSS: legacy sync
	var (
		externTd *big.Int
		current  = bc.CurrentBlock()
	)
...
func (bc *BlockChain) updateFutureBlocks() { // ##CROSS: legacy sync
