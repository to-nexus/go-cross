...
func generateAccounts(ctx *generatorContext, dl *diskLayer, accMarker []byte) error {
	...
	for {
		...
		exhausted, last, err := dl.generateRange(ctx, id, rawdb.SnapshotAccountPrefix, snapAccount, origin, accountCheckRange, onAccount, types.FullAccountRLP) // ##CROSS: UPSTREAM PR-30040
		if err != nil {
			return err // The procedure it aborted, either by external signal or internal error.
		}
