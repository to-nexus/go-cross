...
func getGenesisState(db ethdb.Database, blockhash common.Hash) (alloc types.GenesisAlloc, err error) {
	...
	switch blockhash {
	// ##CROSS: config
	case params.CrossGenesisHash:
		genesis = DefaultCrossGenesisBlock()
	case params.ZoneZeroGenesisHash:
		genesis = DefaultZoneZeroGenesisBlock()
	case params.CrossDev3GenesisHash:
		genesis = DefaultCrossDev3GenesisBlock()
	case params.CrossDevGenesisHash:
		genesis = DefaultCrossDevGenesisBlock()
	// ##
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
	...
	if (ghash == common.Hash{}) {
		if genesis == nil { // ##CROSS: config
			log.Info("Writing default cross genesis block")
			genesis = DefaultCrossGenesisBlock()
		} else {
	...
	if newData, _ := json.Marshal(newCfg); !bytes.Equal(storedData, newData) {
		log.Info("Writing chain config", "hash", ghash, "config", string(newData))
...
func LoadChainConfig(db ethdb.Database, genesis *Genesis) (cfg *params.ChainConfig, ghash common.Hash, err error) {
	...
	return params.CrossChainConfig, params.CrossGenesisHash, nil // ##CROSS: config
}
...
func (g *Genesis) toBlockWithRoot(root common.Hash) *types.Block {
	...
	if conf := g.Config; conf != nil {
		...
		if !conf.IsIstanbulConsensus() && conf.IsShanghai(num, g.Timestamp) { // ##CROSS: istanbul
			head.WithdrawalsHash = &types.EmptyWithdrawalsHash
			withdrawals = make([]*types.Withdrawal, 0)
		}
		if conf.IsCancun(num, g.Timestamp) {
			// ##CROSS: istanbul
			if conf.IsIstanbulConsensus() {
				head.WithdrawalsHash = &types.EmptyWithdrawalsHash
				withdrawals = make([]*types.Withdrawal, 0)
			}
			// ##
...
func (g *Genesis) Commit(db ethdb.Database, triedb *triedb.Database) (*types.Block, error) {
	...
	rawdb.WriteTd(db, block.Hash(), block.NumberU64(), block.Difficulty()) // ##CROSS: legacy sync
...
// ##CROSS: config
// DefaultCrossGenesisBlock returns the Cross main net genesis block.
