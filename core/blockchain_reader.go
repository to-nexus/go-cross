...
// CurrentBlockWithBody retrieves the current head block(with body) of the canonical chain. The
// block is retrieved from the blockchain's internal cache.
func (bc *BlockChain) CurrentBlockWithBody() *types.Block { // ##CROSS: istanbul
	return bc.currentBlockWithBody.Load()
}
...
func (bc *BlockChain) GetTd(hash common.Hash, number uint64) *big.Int { // ##CROSS: legacy sync
