...
func ReadHeaderRLP(db ethdb.Reader, hash common.Hash, number uint64) rlp.RawValue {
	...
	db.ReadAncients(func(reader ethdb.AncientReaderOp) error {
		...
		// ##CROSS: istanbul
		// parse header to make sure we compare using the right hash (QBFT hash is based on a filtered header)
		if len(data) > 0 {
			if header := decodeHeaderRLP(data); header != nil && header.Hash() == hash {
				return nil
			}
		}
		// ##
...
// WriteTdRLP stores the rlp encoded total difficulty of a block into the database.
...
func WriteAncientHeaderChain(db ethdb.AncientWriter, headers []*types.Header, ptd *big.Int) (int64, error) {
	var tdSum = new(big.Int).Set(ptd) // ##CROSS: legacy sync
...
func DeleteBlock(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	...
	DeleteTd(db, hash, number)           // ##CROSS: legacy sync
	DeleteBlobSidecars(db, hash, number) // it is safe to delete non-exist blob // ##CROSS: blob sidecars
}
...
func DeleteBlockWithoutNumber(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	...
	DeleteBody(db, hash, number)
	DeleteTd(db, hash, number)           // ##CROSS: legacy sync
	DeleteBlobSidecars(db, hash, number) // ##CROSS: blob sidecars
}
