...
func TestTdStorage(t *testing.T) { // ##CROSS: legacy sync
	db := NewMemoryDatabase()
...
func TestAncientStorage(t *testing.T) {
	...
	// Write and verify the header in the database
	WriteAncientBlocks(db, []*types.Block{block}, types.EncodeBlockReceiptLists([]types.Receipts{nil}), big.NewInt(100))
...
func TestCanonicalHashIteration(t *testing.T) {
	...
	for i := uint64(1); i <= 8; i++ {
		...
		WriteTd(db, common.Hash{}, i, big.NewInt(10)) // Write some interferential data // ##CROSS: legacy sync
	}
	for i, c := range cases {
...
func BenchmarkWriteAncientBlocks(b *testing.B) {
	...
	batchSidecars := makeTestSidecars(batchSize, blockTxs) // ##CROSS: blob sidecars
	b.ResetTimer()
	...
	for i := 0; i < b.N; i += batchSize {
		...
		// ##
		writeSize, err := WriteAncientBlocks(db, blocks, types.EncodeBlockReceiptLists(receipts), big.NewInt(55))
		if err != nil {
			b.Fatal(err)
		}
		totalSize += writeSize
